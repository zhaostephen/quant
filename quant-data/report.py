...
def get_report_data(year,season):
    ...
    if not storage.fileExists(filename):
        ...
        if r is None:
            return r
        storage.save(ts.get_report_data(year,season),"basics/report_data"+"_"+csv)

    filename = "profit_data"+"_"+csv
    if not storage.fileExists(filename):
        print("get_profit_data")
        storage.save(ts.get_profit_data(year,season),"basics/profit_data"+"_"+csv)

    filename = "operation_data"+"_"+csv
    if not storage.fileExists(filename):
        print("get_operation_data")
        storage.save(ts.get_operation_data(year,season),"basics/operation_data"+"_"+csv)

    filename = "growth_data"+"_"+csv
    if not storage.fileExists(filename):
        print("get_growth_data")
        storage.save(ts.get_growth_data(year,season),"basics/growth_data"+"_"+csv)

    filename = "get_debtpaying_data"+"_"+csv
    if not storage.fileExists(filename):
        print("get_debtpaying_data")
        storage.save(ts.get_debtpaying_data(year,season),"basics/debtpaying_data"+"_"+csv)

    filename = "get_debtpaying_data"+"_"+csv
    if not storage.fileExists(filename):
        print("get_cashflow_data")
        storage.save(ts.get_cashflow_data(year,season),"basics/cashflow_data"+"_"+csv)
